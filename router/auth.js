...
router.post("/addtask", authenticate, async (req,res)=>{
    try{
        const{task,startdate,enddate,priority,taskstatus}=req.body;
        if(!task||!startdate||!enddate||!priority||!taskstatus){
            return res.status(401).json({error:"please fill all data"})
        }
        const usertask= await User.findOne({_id: req.userID})
        if(usertask){
            const usertaskdata = await usertask.addtask(task,startdate,enddate,priority,taskstatus);
            await usertask.save();
